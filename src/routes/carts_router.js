...
const entorno = async() => { 
    ...
    router.put("/:cid/products/:pid", async(request, response) => {
        ...
        try {
            ...
            if(!carrito){
                response.setHeader('Content-Type','application/json');
                return response.status(400).json({status:"error", message:`El carrito con ID: ${cid} no existe`});
            }
        }
        catch(error) {
        ...
        try {
            ...
            if(!producto){
                response.setHeader('Content-Type','application/json');
                return response.status(400).json({status:"error", message:`El producto con ID: ${pid} no existe`});
            }
        }
        catch(error) {
            console.log(error.message)
        }
        console.log(`Desde 120 - El stock actual del producto ${producto.title} es :`, producto.stock);
        //Confirmado existencia de carrito y de producto
        //Verificar si el carrito ya cuenta con el producto
        console.log(carrito.products[0].productId._id, "Desde linea 124");
        ...
        if(ProductoEnCarrito){
            ...
            carrito.products.push({"procudctId": pid, "quantity":cantidad});
        }
            try {
